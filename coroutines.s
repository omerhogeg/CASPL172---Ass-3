...
extern WorldLength,WorldWidth,state,cell,numOfGen,numOfFreq

sys_write:      equ   4 ; we put it
stdout:         equ   1
...
schedular_time:
    mov esi, dword [numOfFreq]
    mov ecx, dword [numOfGen]
    push esi
    push ecx
...
cell_time:
        mov ecx,ebx
        sub ecx,2 ; getting the index of the array
        mov esi,0 ;check the row number

find_values:
        sub ecx,dword[WorldWidth]
        cmp ecx,0
        jl set_col
        inc esi
        jmp find_values
...
push_index:
        push ecx    ;is the col number 
        push esi    ;is the row number
